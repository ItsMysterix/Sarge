...
function checkCompose(filePath: string): CheckResult {
  try {
    const raw = readFileSync(filePath, 'utf8')
    // naive parse of keys under services:
    ...
    for (const line of lines) {
      ...
      if (inServices) {
        if (/^\S/.test(line)) break // out of services block (next top-level)
...
function checkNginxConf(filePath: string): CheckResult {
  try {
...
function checkMigrateScript(): CheckResult {
  try {
