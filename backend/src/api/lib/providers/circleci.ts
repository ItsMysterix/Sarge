...
export class CircleCIProvider implements IProvider {
    id = 'circleci'
    name = 'CircleCI'
    kind: 'static' = 'static'
    valid = true
    ...
    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return { success: true, deploymentId: `cci-${Date.now()}`, metadata: { pipelineTriggered: true }, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'CircleCI pipeline running', logs: [] } }
