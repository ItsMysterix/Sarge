...
export class PusherProvider implements IProvider {
    id = 'pusher'
    name = 'Pusher'
    kind: 'static' = 'static'
    valid = true
    ...
    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return { success: true, deploymentId: `pusher-${Date.now()}`, metadata: {}, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'Pusher integration active', logs: [] } }
