...
export class GitlabProvider implements IProvider {
    ...
    private getToken(creds: Record<string, string>): string {
        return creds.gitlab_token || creds.token || ''
    }

    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return {
            success: true,
            deploymentId: `gl-${Date.now()}`,
            metadata: { pipelineTriggered: true },
            estimatedDuration: 10,
        }
    }

    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: `GitLab pipeline executing`, logs: [] } }
