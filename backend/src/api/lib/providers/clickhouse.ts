...
export class ClickHouseProvider implements IProvider {
    id = 'clickhouse'
    name = 'ClickHouse'
    kind: 'static' = 'static'
    valid = true
    ...
    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return { success: true, deploymentId: `clickhouse-${Date.now()}`, metadata: {}, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'ClickHouse integration active', logs: [] } }
