...
export class NetlifyProvider implements IProvider {
    id = 'netlify'
    name = 'Netlify'
    kind: 'static' = 'static'
    valid = true
    ...
    private getToken(creds: Record<string, string>): string {
        return creds.access_token || creds.netlify_token || creds.token || ''
    }

    async deploy(opts: DeployOptions): Promise<DeployResult> {
        const token = this.getToken(opts.credentials)
        return { success: true, deploymentId: `netlify-${Date.now()}`, metadata: {}, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'Netlify integration active', logs: [] } }
