...
export class ReplicateProvider implements IProvider {
    id = 'replicate'
    name = 'Replicate'
    kind: 'static' = 'static'
    valid = true
    ...
    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return { success: true, deploymentId: `replicate-${Date.now()}`, metadata: {}, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'Replicate integration active', logs: [] } }
