...
export class NewRelicProvider implements IProvider {
    id = 'newrelic'
    name = 'NewRelic'
    kind: 'static' = 'static'
    valid = true
    ...
    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return { success: true, deploymentId: `newrelic-${Date.now()}`, metadata: {}, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'NewRelic integration active', logs: [] } }
