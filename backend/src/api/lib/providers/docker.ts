...
export class DockerProvider implements IProvider {
    id = 'docker'
    name = 'Docker'
    kind: 'static' = 'static' // Registry API
    valid = true
    errors: string[] = []

    private getToken(creds: Record<string, string>): string {
        return creds.docker_token || creds.token || ''
    }
    ...
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: `Docker registry access verified`, logs: [] } }
