...
export class SalesforceProvider implements IProvider {
    id = 'salesforce'
    name = 'Salesforce'
    kind: 'static' = 'static'
    valid = true
    ...
    async deploy(opts: DeployOptions): Promise<DeployResult> {
        return { success: true, deploymentId: 'salesforce-${Date.now()}', metadata: {}, estimatedDuration: 10 }
    }
    async getStatus(opts: StatusOptions): Promise<DeploymentStatus> { return { status: 'success', progress: 100, message: 'Salesforce integration active', logs: [] } }
