...
describe('sarge-cli skeleton', () => {
  it('prints status JSON for fresh workspace', async () => {
    ...
    await runCli(['node', 'sarge', 'status'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
  ...
  it('up -> status shows running', async () => {
    const cap = makeCapture()
    const dir = tmpDataDir()
    await runCli(['node', 'sarge', 'up'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'status'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events = cap.getJson()
  ...
  it('creates a stack with services', async () => {
    ...
    await runCli(
      ...
      { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any }
    )
    const events = cap.getJson()
  ...
  it('snapshot create and replay restores state', async () => {
    ...
    const dir = tmpDataDir()
    await runCli(['node', 'sarge', 'up'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'snapshot', 'create', 's1'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'down'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'snapshot', 'replay', 's1'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'status'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events = cap.getJson()
    const status = events.find((e) => e.msg === 'workspace.status')
    expect(status.data.running).toBe(true)
  ...
  it('detects a repo and creates a plan', async () => {
    ...
    await runCli(['node', 'sarge', 'detect', '--repo', repo], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    ...
    await runCli(['node', 'sarge', 'plan', '--in', bpFile], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
  ...
  it('applies a plan and returns result JSON', async () => {
    ...
    await runCli(['node', 'sarge', 'apply', '--plan', planFile, '--repo', repo], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
  ...
  it('docker on/off toggles config', async () => {
    const cap = makeCapture()
    const dir = tmpDataDir()
    await runCli(['node', 'sarge', 'docker', 'on'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'status'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events = cap.getJson()
    const status = events.find((e) => e.msg === 'workspace.status')
    expect(status.data.dockerEnabled).toBe(true)
    await runCli(['node', 'sarge', 'docker', 'off'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'status'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events2 = cap.getJson()
  ...
  it('logs command tails lines with filters', async () => {
    ...
    await runCli(['node', 'sarge', 'logs', 'api', '--lines', '3', '--include', 'error'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events = cap.getJson()
  ...
  it('env set/get/list works without leaking secrets', async () => {
    ...
    const dir = tmpDataDir()
    // set non-secret
    await runCli(['node', 'sarge', 'env', 'set', 'FOO=bar'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'env', 'get', 'FOO'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events1 = cap.getJson()
    const getEvt = events1.find((e) => e.msg === 'env.get.ok')
    expect(getEvt.data.value).toBe('bar')
    // set secret
    await runCli(['node', 'sarge', 'env', 'set', '--secret', 'PASS=supersecret'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    await runCli(['node', 'sarge', 'env', 'get', 'PASS'], { stdout: cap.stdout, stderr: cap.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
    const events2 = cap.getJson()
    ...
    await runCli(['node', 'sarge', 'env', 'reveal', 'PASS'], { stdout: cap2.stdout, stderr: cap2.stderr }, { SARGE_DATA_DIR: dir, PATH: process.env.PATH || '', NODE_ENV: 'test' as any })
